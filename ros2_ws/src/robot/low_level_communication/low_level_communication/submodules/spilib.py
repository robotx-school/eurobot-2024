"""
Check wiki topic: High level <-> Low level communication protocol

This is spilib_new -> fork of spilib_lite
What's new (pending):
- Splited into 2 parts: SpiInterface - Send and recieve arrays wtih defined sizes between low level and high level; 
    LlcProtocol - high - level control commands, like: move_robot, digitalWrite,,, 
- Better type hinting (using typing lib); cover all
- OOP (refer to code base split)
- check_sensor method deleted (because it depends on new protocol)
- move_robot method deleted - because this is diff_drive/omni_drive node task, not spilib
- move_robot replaced with motor_cotrol_mode system(control velocity - KEEP_SPEED, control steps - FOLLOW_POS)
For more, check wiki pages: Software low level communication, High level <-> Low level communication protocol
TODO write wiki
...
class SpiInterface:
    ...
    def payload_validator(self, buffer: List[int]):
        """
        Check if input array command is correct
        """
        pass
...
class LlcProtocl:
    ...
    def motor_change_control_type(self):
        """
        TODO move to wiki:
        We hace to motors controls modees:
        0. Step control:
            Move stepper motor for some steps count with given velocity and accel. Easy to use for diff drive control
        1. Velocity control
            Set velocies for steppers and they will try to keep it

        motors_velocity_control
        """
        pass

    def motors_step_control(self, motors: Dict[int, List[int]], lock: bool = False) -> None: # check types
        """
        Please refeer to wiki topic: High level <-> Low level communication protocol for more understanding
        """

    def motors_velocity_control(self):
        pass
    
    # def move_robot(dir_, speed=1500, accel=1000, distance=1000):
    #     """
    #     Moves a robot
